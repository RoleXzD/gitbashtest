package me.kaimson.MoonHook.launch;
...
public class MoonHookTweaker implements ITweaker
{
    private final List<String> launchArguments;
    public static final Logger logger;
    
    public MoonHookTweaker() {
    ...
    public void injectIntoClassLoader(final LaunchClassLoader classLoader) {
        ...
        Mixins.addConfiguration("mixins.MoonHook.json");
        if (env.getObfuscationContext() == null) {
            env.setObfuscationContext("notch");
        }
        env.setSide(MixinEnvironment.Side.CLIENT);
        MoonHookTweaker.logger.info("Registering transformers");
        classLoader.registerTransformer("me.kaimson.MoonHook.asm.CameraTransformer");
    }
